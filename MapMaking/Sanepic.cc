...
#define EXITMESS {fprintf(stderr,"%s %d\n",__FILE__,__LINE__);}
#define exit_Sanepic(MyErr) {MPI_Abort(MPI_COMM_WORLD,(int)MyErr);}
...
void do_PtNd(double *PNd, double *sumCross, fftw_complex *fdatas, long *datatopix, double *psip,
	     double *SpN_all, long ns, long ndet, long *indpix, long ipix_min, long npix,
	     bool CORRon, long iseg, long iseg_min, long nseg, unsigned char *segon,
	     long lnr, double *relCalib, double *Mp, long *hits, bool polar, double *polardet,
	     double *polarang, bool GalBP, double *relfGal, int idetr);

void crosspar(double *PNd, long *datatopix, double *psip,
	     long ns, long ndet, long *indpix, long ipix_min, long npix,
	     long iseg, long iseg_min, long nseg, unsigned char *segon, long lnr,
	     bool polar, double *polardet,
	     double *polarang, int idetr);

void compute_ftrProcesdata(double *datas, fftw_complex *fdatas, long ns, long ndet,
			   long iseg, long nseg, long idetr);

void write_tfAS(double *S, fftw_complex *fdatas, long *datatopix, long *indpix,
		double *psip, long npix, long ns, long ndet, long iseg, long iseg_min, long nseg,
		unsigned char *segon, long lnr, bool polar, double *polardet, double *polarang,
		int GalBP, double *relfGal, int idetr);

void deproject(double *S, long *indpix, long *datatopix, double *psip, long ns, long npix,
	       double *Ps, bool polar, double polard, double polara, int GalBP, double rfGal);

void deproject_partial(double *S, long *indpix, long *datatopix, long ns, double *Ps);

void compute_Nm1d (double *d, long ns, double *SpN_all, double relcal, double *Nm1d);

void compute_diagPtNPCorr(double *Nk, long *datatopix, long ns,
			  long *indpix, long ipix_min, int npix, double *dPtNP);
...
void fMPI_merge(double *lmap, double *d, long nsmpix, long npix, long npixtot,
		int rank, int newrank, int size, int nsubm, bool polar, bool GalBP, MPI_Status status,
		bool allprc, MPI_Comm newcomm);
...
double MPI_compute_chi2(fftw_complex *fdatas0, fftw_complex *fodipole,
            fftw_complex *fdatas, fftw_complex *fdata, double *map, long nseg,
			long *datatopix, long *indpix, double *psip, double *relCalib,
			unsigned char *segon, double *SpN_all, long npix, long ns, long ndet,
			long iseg_min, long lnr, int nsubm, bool polar, double *polardet,
            double *polarang, bool GalBP, double *relfGal, long newrank);

void forceLeakzero(double *PtNPmatStot, long nside, long *indpix, long ipixmin, long nsmpix, bool polar);
...
void pointingshift(double x, double y, long nn, double *thetap, double *phip, double *psip, double *theta, double *phi, double *psi);
...
void usage(char *name)
{
  ...
  cerr << "\t[-O <output file>] [-B <bolo ext>] [-G <flag ext>] [-P <pointing ext>]" << endl;
  ...
  cerr << "-f <sample list file name>  file including list of 1st and last samples" << endl;
  ...
  cerr << "-u Noise Nb samp       Number of samples of the noise power spectra" << endl;
  cerr << "-K <File Leak coef>    Name of file containing the coefficients of the Leakage component" << endl;
  cerr << "-X <det location file> File providing pointing info for each detector wrt boresight"<< endl;
  ...
  cerr << "-p <polar>             set this keyword if polarization is included" << endl;
  cerr << "-b <extra component>   set this keyword if extra component is estimated" << endl;
  cerr << "-l <data seg length>   Lenght of data segments. Must be the same for all segments" << endl; 
  cerr << "-n <nb of segments>    Number of data segments" << endl; 
  cerr << "-N <Nside>             Healpix nside" << endl;
  cerr << "-R <Global recalib>    Global recalibration coef for each detector" << endl;
  cerr << "-r <recalib>           If set, recalibrate data on some timescale (to implement)" << endl;
  cerr << "-c <recalib leakage>   If set, recalibrate leakage component" << endl;
  cerr << "-j <Delt iter>         Number of iterations after which the maps are recalculated" << endl;
  cerr << "-L <no baseline>       Keyword specifiying if a baseline is removed from the data or not (0 for YES, 1 for NO)" << endl;
  cerr << "-z <precompute PNd>    Keyword specifying if PNd is precomputed and read from disk" << endl;
...
int main(int argc, char *argv[])
{
  ...
  double xsh, ysh;
  int iterw = 10;
  bool CORRon = 0, polar = 0, GalBP = 0, recalLeak = 0, recalib = 0, applycalib= 0, recalibGlobal = 0;
  ...
  double *PNd, *PNdtot, *SpN_all=NULL, *SpN=NULL, *polardet=NULL, *polarang=NULL, *relfGal=NULL;
  ...
  while ( (retval = getopt(argc, argv, "F:Z:f:n:l:d:D:C:H:h:w:J:o:O:B:R:X:G:P:e:p:A:m:k:K:u:c:N:n:L:g:r:M:E:j:z:b:")) != -1) {
    switch (retval) {
    case 'F':
      dirfile = optarg;
      break;
  ...
  for (idet=0;idet<ndet;idet++){
    ...
    offx[idet] = offsets[0];///180.0*D2PI/2.0;
    offy[idet] = offsets[1];///180.0*D2PI/2.0;
...
MPI_Barrier(MPI_COMM_WORLD);
  ...
  if (CORRon){
    ...
    for (idet=0;idet<ndet;idet++){
      ...
      sprintf(NoiseSpFiledet,"%s%s.bi",noiseSppreffile.c_str(),bolo.c_str());
  printf("NoiseSpFiledet = %s\n",NoiseSpFiledet);    
	...
	printf("ns = %ld, %d\n",ns,MyErr);
      ...
      for (ii=0;ii<ns;ii++)
	SpN_all[ii + idet*ns + idet*ndet*ns] = SpN[long(double(ii)/nssp*ns)]/double(ns)/double(ns);
      SpN_all[idet*ns + idet*ndet*ns] = SpN[1]*1e-6/double(ns)/double(ns); 
    }
  }
  ...
  //if (0 & polar)
    //MyErr = PIOReadVECTObject((void **)&polardet, Param->Polardet, (char *) "PIODOUBLE",  (const char *) "", NULL);
  ...
  if (GalBP){
    ...
    for (idet=0;idet<ndet;idet++)
      ttsig += (relfGal[idet]-ttmean)*(relfGal[idet]-ttmean)/double(ndet);
    for (idet=0;idet<ndet;idet++)
...
printf("BEFORE READING DATA\n");
  ...
  if (1){
    ...
    for (iseg=iseg_min;iseg<=iseg_max;iseg++){
      ...
      if (isampmax[iseg]-isampmin[iseg] + 1 != ns){
        printf("Problem with segment size\n");
	    exit(1);
      }
      ...
      MyErr=ReadDoubleVECT(decs, namefilering, isampmin[iseg], isampmax[iseg]);
      
      if (polar){
	sprintf(namefilering,"%spsi%s.bi",dirfilep.c_str(),pextension.c_str());
	MyErr=ReadDoubleVECT(psips, namefilering, isampmin[iseg], isampmax[iseg]);
	...
	if (segon[idet*lnr + iseg]){
	  ...
	  if (recalib || odip){
	    ...
	    sprintf(namefilering,"%s/orbdip_%s",dirfile.c_str(),bolo.c_str());
	    MyErr=ReadDoubleVECT(orbdata, namefilering, isampmin[iseg], isampmax[iseg]);
	    
	    fftplan = fftw_plan_dft_r2c_1d(ns, orbdata, fdata, FFTW_ESTIMATE);
	    fftw_execute(fftplan);
	    fftw_destroy_plan(fftplan);
	    for (ii=0;ii<ns/2+1;ii++){
	      fodipole[ii+idet*(ns/2+1)+(iseg-iseg_min)*ndet*(ns/2+1)][0] = fdata[ii][0];
	      fodipole[ii+idet*(ns/2+1)+(iseg-iseg_min)*ndet*(ns/2+1)][1] = fdata[ii][1];
	    }
	  }
	  ...
	  /* Compute pointing for each detector */   //// need to add parameters from the file
	  ...
	  for (ii=0;ii<ns;ii++){
	    ...
	    phip_i = ras[ii]; 
	    psip_i = psips[ii];
	    //pointingshift(-offx[idet],-offy[idet],1, &thetap_i, &phip_i, &psip_i, &theta_o, &phi_o, &psi_o);  // To match with slaDtp2s()
	    pointingshift(offx[idet],offy[idet],1, &thetap_i, &phip_i, &psip_i, &theta_o, &phi_o, &psi_o); // Offset on the sky I am guessing
	    
	    //if (ii==100)
	    //printf("idet=%ld, offx=%10.15g, offy=%10.15g\n",idet,offx[idet],offy[idet]);
	    ra_rad = phi_o;
	    dec_rad = D2PI/4.0-theta_o;
	    
	    ra[ii+ns*(iseg-iseg_min)+ns*nseg*idet] = ra_rad;   //// assuming radians
	    dec[ii+ns*(iseg-iseg_min)+ns*nseg*idet] = dec_rad;  ///// assuming radians
	    //ra[ii+ns*(iseg-iseg_min)+ns*nseg*idet] = ras[ii];   //// assuming radians
	    //dec[ii+ns*(iseg-iseg_min)+ns*nseg*idet] = decs[ii];  ///// assuming radians
	    
	    
	    if (polar){
	      psip[ii+ns*(iseg-iseg_min)+ns*nseg*idet] = psi_o + psi[idet]; //// add angle
	      if (WriteCrossLink)
                psipFP[ii+ns*(iseg-iseg_min)+ns*nseg*idet] = psip[ii+ns*(iseg-iseg_min)+ns*nseg*idet];
	      //if (ii == 1000)
	      //printf("psip[1000] = %10.15g\n",psip[1000]);
	      if (HWP)
		psip[ii+ns*(iseg-iseg_min)+ns*nseg*idet] = -(psi_o + psi[idet]) + 2.0 * OmegaHWP * (ii+ns*iseg) ;
	      
	      if ((ii < 0) & (iseg ==0))
		printf("psip[%ld]=%10.15g, angle bolo=%10.15g\n",ii+ns*(iseg-iseg_min)+ns*nseg*idet, psip[ii+ns*(iseg-iseg_min)+ns*nseg*idet],psi[idet]);
	    }
	  }
	  
	  //printf("psip[1895654] = %10.15g\n",psip[1895654+ns*(iseg-iseg_min)+ns*nseg*idet]);
	  
	  /* read all input object only */
	  data = new double[ns];
	  sprintf(namefilering,"%s/data_%s%s.bi",dirfile.c_str(),bolo.c_str(),bextension.c_str());
      for (ii=0;ii<ns;ii++)
            if (!(data[ii] > 0) && !(data[ii] <= 0.0)){
                printf("NAN DETECTED IN data, data = %10.15g\n",data[ii]);
                if (ii > 0)
                    data[ii] = data[ii-1];
            }
      ...
      if (iseg == iseg_min)
        printf("%s\n",namefilering);
	  MyErr=ReadDoubleVECT(data, namefilering, isampmin[iseg], isampmax[iseg]);
	  ...
	  fftw_destroy_plan(fftplan);
	  for (ii=0;ii<ns/2+1;ii++){
	    fdatas0[ii+idet*(ns/2+1)+(iseg-iseg_min)*ndet*(ns/2+1)][0] = fdata[ii][0];
	    fdatas0[ii+idet*(ns/2+1)+(iseg-iseg_min)*ndet*(ns/2+1)][1] = fdata[ii][1];
	  }
	 //if ((rank == 1) & (iseg == iseg_min) & (idet == 0)){
     //   FILE *fp=fopen("fdata.dat","w");
     //   fwrite(fdata, sizeof(double)*ns, 1,fp);
     //   fclose(fp);
     //}
	  if (recalib)  //// or odip??
	    delete(orbdata);
	...
	if (segon[idet*lnr + iseg+iseg_min])
	  for (ii=0;ii<ns;ii++){
	    ...
	    if (ipr < 0 || ipr > 12*nside*nside-1){
	      printf("PROBLEM WITH PIXEL INDEX: ii = %ld, ipr=%ld, ra = %10.15g, dec=%10.15g\n",ii,ipr,dec[ii+ns*iseg+ns*nseg*idet],ra[ii+ns*iseg+ns*nseg*idet]);
	      ipr=ipr_;
	      printf("PROBLEM WITH PIXEL INDEX: ii = %ld, ipr=%ld, ra = %10.15g, dec=%10.15g\n",ii-1,ipr,dec[ii-1+ns*iseg+ns*nseg*idet],ra[ii-1+ns*iseg+ns*nseg*idet]);
  ...
  MPI_Reduce(pixon,pixonall,12*nside*nside,MPI_DOUBLE,MPI_SUM,0,newcomm);
  ...
  npixtot = npix;
  if (polar) npixtot += 2*npix;
  if (GalBP) npixtot += npix;
  ...
  if (polar) nsmpixtot += 2*nsmpix;
  if (GalBP) nsmpixtot += nsmpix;
  ...
  printf("TOTAL NUMBER OF FILLED PIXELS IN ONE MAP: %ld, TOTAL NUMBER: %ld, FOR PROCESSOR %d: %ld, NB OF HEALPIX PIXEL: %ld \n",npix,npixtot,rank,nsmpixtot,12*nside*nside);
  ...
  if (Destrip /* || simul */){
    for (jj=0;jj<ndet;jj++)
      for (kk=0;kk<ndet;kk++){
//	  if (simul) {
//	      varN = Param->simulSigmaNoise[idet] * Param->simulSigmaNoise[idet];
//	      if (varN == 0.) varN = 1.;
//	  } else {
...
//	  }
    ...
    for (jj=0;jj<ndet;jj++)
      SpN_all[jj*ns+jj*ns*ndet] = SpN_all[jj*ns+jj*ns*ndet]*1e-6;
  ...
  int iter;
  double var0, var00, var_n, var_n_, delta0, delta_n, delta_n_, delta_o, rtq, rtqt, alpha, beta, chi2, chi20, chi2tot, absrecal, absrecal_old;
  double *S, *tmpmap, *PtNPmatStot, *d, *Mp, *Mptot, *sumCross, *sumCrosstot, *qtot, *relCalib, *relCalib_old, *zeta, *gamma, *zeta_all, *gamma_all;
  ...
  if (rank == 0)
    ...
    for (idet=0;idet<ndet;idet++){
      ...
      // MyErr = ReadVECT((void **)&tmpval, Param->coefcalib[idet], 8, 0, ndet-1);
  ...
  while (rloop && rloop < 500 && var0/var00 > StopCriterium){
	...
	for (idet=0;idet<ndet;idet++)
	  if (segon[idet*lnr + iseg+iseg_min])
	    for (ii=0;ii<ns/2+1;ii++){
	      fdatas0[ii+idet*(ns/2+1)+iseg*ndet*(ns/2+1)][0] /= relCalib[idet + iseg+iseg_min*ndet];
	      fdatas0[ii+idet*(ns/2+1)+iseg*ndet*(ns/2+1)][1] /= relCalib[idet + iseg+iseg_min*ndet];
	    }
    }
	...
	for (iseg=0;iseg<nseg;iseg++)
	  if (segon[idet*lnr + iseg+iseg_min])
	    for (ii=0;ii<ns/2+1;ii++){
	      fdatas0[ii+idet*(ns/2+1)+iseg*ndet*(ns/2+1)][0] -= fodipole[ii+idet*(ns/2+1)+iseg*ndet*(ns/2+1)][0];
	      fdatas0[ii+idet*(ns/2+1)+iseg*ndet*(ns/2+1)][1] -= fodipole[ii+idet*(ns/2+1)+iseg*ndet*(ns/2+1)][1];
	    }
    }
	...
	for (idet=0;idet<ndet;idet++)
	  if (segon[idet*lnr + iseg+iseg_min])
	    for (ii=0;ii<ns/2+1;ii++){
	      fdatas[ii+idet*ns][0] = fdatas0[ii+idet*(ns/2+1)+iseg*ndet*(ns/2+1)][0];
	      fdatas[ii+idet*ns][1] = fdatas0[ii+idet*(ns/2+1)+iseg*ndet*(ns/2+1)][1];
	    }
	...
	for (idet=0;idet<ndet;idet++){
	  ...
	  if (segon[idet*lnr + iseg+iseg_min]){
	    for (ii=0;ii<ns/2+1;ii++){
	      fdata[ii][0] = fdatas[ii+idet*ns][0]*sqrt(SpN_all[ii+idet*ns+idet*ndet*ns])*relCalib[idet+(iseg+iseg_min)*ndet];
	      fdata[ii][1] = fdatas[ii+idet*ns][1]*sqrt(SpN_all[ii+idet*ns+idet*ndet*ns])*relCalib[idet+(iseg+iseg_min)*ndet];
	    }

        
        //if ((rank == 1) & (iseg==0) & (idet==0)){
        //    FILE *fp=fopen("SpN.dat","w");
        //    fwrite(SpN_all+idet*ns+idet*ndet*ns, sizeof(double)*ns, 1,fp);
        //    fclose(fp);
        //}
	    ...
	    for (ii=0;ii<ns/2+1;ii++)
	      chi20 += (fdata[ii][0] * fdata[ii][0] + fdata[ii][1] * fdata[ii][1])/double(nsubm)*2.0;
	  }
	}
	
	do_PtNd(PNd, NULL, fdatas, datatopix, psip, SpN_all, ns, ndet, indpix, ipixmin, nsmpix,
		CORRon, iseg, iseg_min, nseg, segon, lnr, relCalib+(iseg+iseg_min)*ndet,
		NULL, NULL, polar, polardet, polarang, GalBP, relfGal, -1);
    ...
    if (0 & (newrank == 0)){  
      ...
      fMPI_merge(tmpinitmap2,PNdtot,nsmpix,npix,npixtot,rank,newrank,size,nsubm,polar,GalBP,status,0,mastercomm);
      if (rank == 0)
    ...
    if (rloop == 1){///////  Do sizes match here between S and tmpinitmap ?????
      tmpinitmap = new double[npixtot];
      if (nsubm > 1)
	fMPI_merge(tmpinitmap,S,nsmpix,npix,npixtot,rank,newrank,size,nsubm,polar,GalBP,status,1,MPI_COMM_WORLD);
      else {
	...
	MyErr=ReadVECT((void **)&tmpinitmap, filn, 8, 0, npixtot);
    ...
    //////////////////////////// force leakage to be 0 at high galactic latitude
    if (recalib && GalBP)
      forceLeakzero(S, nside, indpix, ipixmin, nsmpix, polar);  ///why tmpinitmap is not forced to 0??
	...
	write_tfAS(tmpinitmap,fdatas,datatopix,indpix,psip,npix,ns,ndet,iseg,iseg_min,nseg,segon,
		   lnr,polar,polardet,polarang,GalBP,relfGal,-1);
	
	if (rloop == 1)
        if (WriteCrossLink)
            do_PtNd(tmpmap,sumCross,fdatas,datatopix,psipFP,SpN_all,ns,ndet,indpix,ipixmin,nsmpix,CORRon,
		            iseg,iseg_min,nseg,segon,lnr,relCalib+(iseg+iseg_min)*ndet,Mp,hits,polar,
		            polardet,polarang,GalBP,relfGal,-1);
        else
	        do_PtNd(tmpmap,NULL,fdatas,datatopix,psip,SpN_all,ns,ndet,indpix,ipixmin,nsmpix,CORRon,
		            iseg,iseg_min,nseg,segon,lnr,relCalib+(iseg+iseg_min)*ndet,Mp,hits,polar,
		            polardet,polarang,GalBP,relfGal,-1);
	else
	  do_PtNd(tmpmap,NULL,fdatas,datatopix,psip,SpN_all,ns,ndet,indpix,ipixmin,nsmpix,CORRon,
		  iseg,iseg_min,nseg,segon,lnr,relCalib+(iseg+iseg_min)*ndet,NULL,NULL,
		  polar,polardet,polarang,GalBP,relfGal,-1);  

    //crosspar(mapscossin, datatopix, psip, ns, ndet, indpix, ipix_min, npix,
	...
	delete [] hitstot;
      ...
      MPI_Reduce(sumCross,sumCrosstot,nsmpixtot,MPI_DOUBLE,MPI_SUM,0,newcomm);
      if (newrank != 0){
        delete [] Mptot;
        delete [] sumCrosstot;
      }
	...
	if (ii >= nsmpix) {
	  if (polar && (ii < 3*nsmpix) && (hitstot[(ii % nsmpix) + nsmpix] < 4)){
	    PtNPmatStot[ii] = 0.0;
	    S[ii] = 0.0;
	    PNdtot[ii] = 0.0;
	    if (rloop == 1){
    ...
    if (newrank == 0){
      ...
      printf("PtNPmatStot*PtNPmatStot = %10.15g\n",var_n);
      ...
      if (rloop == 1){
	    var_n = 0.0;
	    for (ii=0;ii<nsmpixtot;ii++)
	        var_n += PNdtot[ii]*PNdtot[ii];
	    if (nsubm > 1)
	        MPI_Reduce(&var_n,&var00,1,MPI_DOUBLE,MPI_SUM,0,mastercomm);
	    else
	        var00 = var_n;
      }
    ...
    while((iter < 2000 && var_n/var00 > StopCriterium) || iter == 0){
      ...
      tmpinitmap = new double[npixtot];
      if (nsubm > 1)
	fMPI_merge(tmpinitmap,d,nsmpix,npix,npixtot,rank,newrank,size,nsubm,polar,GalBP,status,1,MPI_COMM_WORLD);
      else {
	for (ii=0;ii<npixtot;ii++)
	  tmpinitmap[ii] = d[ii];
      }
	...
	for (iseg=0;iseg<nseg;iseg++){
	  ...
	  do_PtNd(tmpmap,NULL,fdatas,datatopix,psip,SpN_all,ns,ndet,indpix,ipixmin, nsmpix,CORRon,
		  iseg,iseg_min,nseg,segon,lnr,relCalib+(iseg+iseg_min)*ndet,NULL,NULL,polar,
		  polardet,polarang,GalBP,relfGal,-1);  
	} // end of iseg loop
	...
	delete [] qtot;
      ...
      if (newrank == 0){ 
	// remove degeneracies by forcing to 0 the Q and U components for which the nb of obs is < 4
	for (ii=0;ii<nsmpixtot;ii++)
	  if (ii >= nsmpix)
	    if (polar && (ii < 3*nsmpix) && (hitstot[(ii % nsmpix) + nsmpix] < 4))
	      qtot[ii] = 0.0;
	...
	if (newrank != 0){
	  for (iseg=0;iseg<nseg;iseg++){
	    ...
	    write_tfAS(tmpinitmap,fdatas,datatopix,indpix,psip,npix,ns,ndet,iseg,iseg_min,nseg,
		       segon,lnr,polar,polardet,polarang,GalBP,relfGal,-1);
		...
		// compute first term of chi2
		for (ii=0;ii<ns/2+1;ii++){
		  fdata[ii][0] = (fdatas0[ii+idet*(ns/2+1)+iseg*ndet*(ns/2+1)][0] - fdatas[ii+idet*ns][0])*sqrt(SpN_all[ii+idet*ns+idet*ndet*ns])*relCalib[idet+(iseg+iseg_min)*ndet];
		  fdata[ii][1] = (fdatas0[ii+idet*(ns/2+1)+iseg*ndet*(ns/2+1)][1] - fdatas[ii+idet*ns][1])*sqrt(SpN_all[ii+idet*ns+idet*ndet*ns])*relCalib[idet+(iseg+iseg_min)*ndet];
		}
		
		for (ii=0;ii<ns/2+1;ii++)
		  chi2 += (fdata[ii][0] * fdata[ii][0] + fdata[ii][1] * fdata[ii][1])/double(nsubm)*2.0;
	      }
	    }
	  } // end of iseg loop
	}
	...
	if (nsubm > 1){
	  MPI_Reduce(&delta_n_,&delta_n,1,MPI_DOUBLE,MPI_SUM,0,mastercomm);
	...
	sprintf(mapoutstr, "%s_NVAR_%s.bi", outdir.c_str(),termin_out.c_str());
	tmpinitmap = new double[npixtot];
	if (nsubm > 1)
	  fMPI_merge(tmpinitmap,Mptot,nsmpix,npix,npixtot,rank,newrank,size,nsubm,polar,GalBP,status,0,mastercomm);
	else {
	  for (ii=0;ii<npixtot;ii++)
	...
	sprintf(mapoutstr, "%s_HITS_%s.bi", outdir.c_str(),termin_out.c_str());
	...
	if (nsubm > 1){
	  ...
	  if (rank == 0){
	    ...
	    for (idet=1;idet<nsubm;idet++){
	      MPI_Recv(tmptabl+idet*npix/nsubm,-idet*npix/nsubm + (idet+1)*npix/nsubm,MPI_LONG,idet*size/nsubm,idet*size/nsubm,MPI_COMM_WORLD,&status);
	    }
	    for (ii=0;ii<npix;ii++)
	  ...
	  write_reduced_map(mapoutstr,nside,tmptab,indpix);
      sprintf(mapoutstr, "%s_COS2P_%s.bi", outdir.c_str(),termin_out.c_str());
      write_reduced_map(mapoutstr,nside,sumCrosstot,indpix);
      sprintf(mapoutstr, "%s_SIN2P_%s.bi", outdir.c_str(),termin_out.c_str());
	...
	if (polar){
	  ...
	  if (rank == 0){
	    sprintf(mapoutstr, "%s_Q_NVAR_%s.bi", outdir.c_str(),termin_out.c_str());
	    write_reduced_map(mapoutstr,nside,tmpinitmap+npix,indpix);
	    sprintf(mapoutstr, "%s_U_NVAR_%s.bi", outdir.c_str(),termin_out.c_str());
	    write_reduced_map(mapoutstr,nside,tmpinitmap+2*npix,indpix);
	  }
	  if (nsubm > 1){
	    if (rank != 0)
	      MPI_Send(hitstot+nsmpix, nsmpix, MPI_LONG, 0, rank, MPI_COMM_WORLD);
	    if (rank == 0){
	      for (ii=0;ii<nsmpix;ii++)
		...
		MPI_Recv(tmptabl+idet*npix/nsubm,-idet*npix/nsubm + (idet+1)*npix/nsubm,MPI_LONG,idet*size/nsubm,idet*size/nsubm,MPI_COMM_WORLD,&status);
	      for (ii=0;ii<npix;ii++)
		tmptab[ii] = double(tmptabl[ii]);
	      sprintf(mapoutstr, "%s_POL_HITS_%s.bi", outdir.c_str(),termin_out.c_str());
	      write_reduced_map(mapoutstr,nside,tmptab,indpix);
	    }
	  } else {
	    /// just copy data 
	  ...
	  if (GalBP && (rank == 0)){
	    sprintf(mapoutstr, "%s_Leak_NVAR_%s.bi", outdir.c_str(),termin_out.c_str());
	...
	sprintf(mapoutstr, "%s%d.bi", outdir.c_str(), iter);
	tmpinitmap = new double[npixtot];
	if (nsubm > 1)
	  fMPI_merge(tmpinitmap,S,nsmpix,npix,npixtot,rank,newrank,size,nsubm,polar,GalBP,status,0,mastercomm);
	else {
	  for (ii=0;ii<npixtot;ii++)
	    tmpinitmap[ii] = S[ii];
	}
	if (rank == 0)
	  sprintf(mapoutstr, "%s_I_%s_%d.bi", outdir.c_str(), termin_out.c_str(), iter);
	  write_reduced_map(mapoutstr,nside,tmpinitmap,indpix);
	
	if (polar && (rank == 0)){
	  
	  sprintf(mapoutstr, "%s_Q_%s_%d.bi", outdir.c_str(),termin_out.c_str(), iter);
	  write_reduced_map(mapoutstr,nside,tmpinitmap+npix,indpix);
	  sprintf(mapoutstr, "%s_U_%s_%d.bi", outdir.c_str(),termin_out.c_str(), iter);
	  write_reduced_map(mapoutstr,nside,tmpinitmap+2*npix,indpix);	
	  
	  if (GalBP && (rank == 0)){
	    sprintf(mapoutstr, "%s_Leak%s_%d.bi", outdir.c_str(),termin_out.c_str(), iter);
	    write_reduced_map(mapoutstr,nside,tmpinitmap+3*npix,indpix);
	  }
	} else if (GalBP && (rank == 0)){
	  sprintf(mapoutstr, "%s_Leak%s_%d.bi", outdir.c_str(),termin_out.c_str(), iter);
	...
	for (ii=0;ii<npixtot;ii++)
      ...
      if (rank == 0){
	sprintf(mapoutstr, "%s_I_%s_%d.bi", outdir.c_str(),termin_out.c_str(),rloop);
	write_reduced_map(mapoutstr,nside,tmpinitmap,indpix);
	
	if (polar){
	  
	  sprintf(mapoutstr, "%s_Q_%s_%d.bi", outdir.c_str(),termin_out.c_str(),rloop);
	  write_reduced_map(mapoutstr,nside,tmpinitmap+npix,indpix);
	  sprintf(mapoutstr, "%s_U_%s_%d.bi", outdir.c_str(),termin_out.c_str(),rloop);
	  write_reduced_map(mapoutstr,nside,tmpinitmap+2*npix,indpix);	
	  
	  if (GalBP){
	    sprintf(mapoutstr, "%s_Leak_%s_%d.bi", outdir.c_str(),termin_out.c_str(),rloop);
	    write_reduced_map(mapoutstr,nside,tmpinitmap+3*npix,indpix);
	  }
	} else if (GalBP){
	  sprintf(mapoutstr, "%s_Leak_%s_%d.bi", outdir.c_str(),termin_out.c_str(),rloop);
    ...
    if (((GalBP && recalLeak) || (polar && recalPolar))){
      ...
      // For this, need to compute r_{0,1,2,3} = N^(-1) Ps for s = I, GalBP, Q', U'
      // where Q' and U' are Q,U rotated to focalplane reference frame
      // From these 3 TOI-length data r_{1,2,3}, compute b_i = sum([m-r0].r_i)
      // and M_ij = sum(r_i.r_j)
      // And finally solve for K = M^(-1).b
	...
	polarang_n = new double[ndet];
      ...
      for (idet = 0; idet < ndet; idet ++){  // Calibration for each detector is independent
	if (newrank != 0) {
	  for (iseg = iseg_min; iseg <= iseg_max; iseg ++)  {
	    //void deproject_partial(double *S, long *indpix, long *datatopix, long ns, double *Ps)
	    if (segon[idet*lnr + iseg])
	      deproject_partial (tmpinitmap,indpix,
				 datatopix + ns * (iseg - iseg_min) + ns * nseg * idet,
	...
	if (recalLeak && !recalPolar) {   // Simple case : scalar
	  ...
	  if (newrank != 0){
		...
		compute_Nm1d (r1 + ns * (iseg - iseg_min),
			      ...
			      recalib ? relCalib[idet + iseg * ndet] : 1., Nm1r1);
		for (ii = 0; ii < ns; ii ++) {
		  long idx;
		  double Ps;
		  idx = (iseg - iseg_min) * ns + ii;
		  Ps = r0[idx];
		  if (polar) Ps += polardet[idet] * 
			       (r2[idx] * cos (2 * polarang[idet]) +
				r3[idx] * sin (2 * polarang[idet]));
		  A_ += r1[idx] * Nm1r1[ii];
		  //b_ += (datas[idx + idet * ns * nseg] - Ps) * Nm1r1[ii];  ////////  Implement fitting in Fourier space
		}
	      }
	    }
	    delete [] Nm1r1;
	  ...
	  // DEBUG
	  if (newrank == 1)
	    printf ("rank=%d : idet = %d : A_ = %.10lg   b_ = %.10lg   A = %.10lg   b = %.10lg\n",
		    rank, (int)idet, A_, b_, A, b);
	}
	if (!recalLeak && recalPolar){ // Need to build a 2x2 matrix and solve the system
	  /*
	  ...
	  if (newrank != 0){
		...
		compute_Nm1d (r2 + ns * (iseg - iseg_min), ns,
			      ...
			      recalib ? relCalib[idet + iseg * ndet] : 1., Nm1r2);
		compute_Nm1d (r3 + ns * (iseg - iseg_min),
			      ns, SpN_all + idet * ns + idet * ndet * ns,
			      recalib ? relCalib[idet + iseg * ndet] : 1., Nm1r3);
		for (ii = 0; ii < ns; ii ++){
		  long idx;
		  double Ps;
		  idx = (iseg - iseg_min) * ns + ii;
		  Ps = r0[idx];
		  if (GalBP) Ps += r1[idx];
		  A_[0] += r2[idx] * Nm1r2[ii];
		  A_[1] += r2[idx] * Nm1r3[ii];
		  A_[3] += r3[idx] * Nm1r3[ii];
		  b_[0] += (datas[idx + idet * ns * nseg] - Ps) * Nm1r2[ii];
		  b_[1] += (datas[idx + idet * ns * nseg] - Ps) * Nm1r3[ii];
		}
	      }
	    }
	    delete [] Nm1r2;
	...
	if (recalLeak && recalPolar){ // Need to build a 3x3 matrix and solve the system
	  /*
	  ...
	  if (newrank != 0){
		...
		compute_Nm1d (r1 + ns * (iseg - iseg_min),
			      ns, SpN_all + idet * ns + idet * ndet * ns,
			      recalib ? relCalib[idet + iseg * ndet] : 1., Nm1r1);
		compute_Nm1d (r2 + ns * (iseg - iseg_min),
			      ns, SpN_all + idet * ns + idet * ndet * ns,
			      recalib ? relCalib[idet + iseg * ndet] : 1., Nm1r2);
		compute_Nm1d (r3 + ns * (iseg - iseg_min),
			      ns, SpN_all + idet * ns + idet * ndet * ns,
		...
		for (ii = 0; ii < ns; ii ++){
		  long idx;
		  idx = (iseg - iseg_min) * ns + ii;
		  A_[0] += r1[idx] * Nm1r1[ii];
		  A_[1] += r1[idx] * Nm1r2[ii];
		  A_[2] += r1[idx] * Nm1r3[ii];
		  A_[4] += r2[idx] * Nm1r2[ii];
		  A_[5] += r2[idx] * Nm1r3[ii];
		  A_[8] += r3[idx] * Nm1r3[ii];
		  b_[0] += (datas[idx + idet * ns * nseg] - r0[idx]) * Nm1r1[ii];
		  b_[1] += (datas[idx + idet * ns * nseg] - r0[idx]) * Nm1r2[ii];
		  b_[2] += (datas[idx + idet * ns * nseg] - r0[idx]) * Nm1r3[ii];
		}
	      }
	    }
	    delete [] Nm1r1;
	...
	for (idet=0; idet < ndet; idet ++)
	  ttsig += (relfGal_n[idet] - ttmean) * (relfGal_n[idet] - ttmean) / double(ndet);
	for (idet = 0; idet < ndet; idet ++)
	...
	if (rank == 0)
	  for (idet = 0; idet < ndet; idet ++)
	    printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!! polardet[%ld] = %10.15g    polarang[%ld] = %10.15g\n", idet, polardet[idet], idet, polarang[idet] * 180 / M_PI);
      }
	
      // Free all allocated memory, except tmpinitmap
      if (r0) delete [] r0;
      if (relfGal_n) delete [] relfGal_n;
      if (r1) delete [] r1;
      if (r2) delete [] r2;
      if (r3) delete [] r3;
      if (polardet_n) delete [] polardet_n;
      if (polarang_n) delete [] polarang_n;
	...
	for (iseg=0;iseg<nsegtot;iseg++)
      ...
      absrecal = 1.0;
      int niterlim = 1;
      if (rloop < 6) niterlim = 50;
      
      for (int rerun=0;rerun < niterlim;rerun++){
	
	/// compute chi2
	chi2 = MPI_compute_chi2(fdatas0,fodipole,fdatas,fdata,tmpinitmap,nseg,datatopix,indpix,psip,relCalib,segon,SpN_all,npix,ns,ndet,iseg_min,lnr,nsubm,polar,polardet,polarang,GalBP,relfGal,newrank);
	if (rank == 0)
	...
	if (newrank != 0){
	  for (idet=0;idet<ndet;idet++){
		...
		// Compute Ps
		deproject(tmpinitmap,indpix,datatopix+iseg*ns+ns*nseg*idet,
			  psip+iseg*ns+ns*nseg*idet,ns,npix,Nd,polar,*(polardet+idet),*(polarang+idet),
			  GalBP,*(relfGal+idet));
		...
		for (ii=0;ii<ns/2+1;ii++){
		  ...
		  sPNd += (fdatas0[pfd][0]*(fdata[ii][0]/absrecal + fodipole[pfd][0])  +  fdatas0[pfd][1]*(fdata[ii][1]/absrecal + fodipole[pfd][1]))*SpN_all[ii+idet*ns+idet*ndet*ns] / double(nsubm) * 2.0;
		}
		
		for (ii=0;ii<ns/2+1;ii++){
		  fdata[ii][0] = (fdata[ii][0]/absrecal + fodipole[ii+idet*(ns/2+1)+iseg*ndet*(ns/2+1)][0])*sqrt(SpN_all[ii+idet*ns+idet*ndet*ns]);
		  fdata[ii][1] = (fdata[ii][1]/absrecal + fodipole[ii+idet*(ns/2+1)+iseg*ndet*(ns/2+1)][1])*sqrt(SpN_all[ii+idet*ns+idet*ndet*ns]);
		}
		
		//// computation of cPNPc with an alternative approach
		for (ii=0;ii<ns/2+1;ii++)
		  cPNPc += (fdata[ii][0]*fdata[ii][0] + fdata[ii][1]*fdata[ii][1]) / 2.0;
	...
	// Merge information
	MPI_Reduce(relCalib,relfI_n,ndet*nsegtot,MPI_DOUBLE,MPI_SUM,0,MPI_COMM_WORLD);
	MPI_Bcast(relfI_n, ndet*nsegtot,MPI_DOUBLE,0,MPI_COMM_WORLD);
	MPI_Barrier(MPI_COMM_WORLD);
	for (ii=0;ii<ndet*nsegtot;ii++)
	  relCalib[ii] = relfI_n[ii]/double(nsubm);
	
	MPI_Reduce(zeta,relfI_n,ndet*nsegtot,MPI_DOUBLE,MPI_SUM,0,MPI_COMM_WORLD);
	MPI_Bcast(relfI_n, ndet*nsegtot,MPI_DOUBLE,0,MPI_COMM_WORLD);
	MPI_Barrier(MPI_COMM_WORLD);
	for (ii=0;ii<ndet*nsegtot;ii++)
	  zeta[ii] = relfI_n[ii];
	
	MPI_Reduce(gamma,relfI_n,ndet*nsegtot,MPI_DOUBLE,MPI_SUM,0,MPI_COMM_WORLD);
	MPI_Bcast(relfI_n, ndet*nsegtot,MPI_DOUBLE,0,MPI_COMM_WORLD);
	MPI_Barrier(MPI_COMM_WORLD);
	for (ii=0;ii<ndet*nsegtot;ii++)
	  gamma[ii] = relfI_n[ii];
	...
	//delete [] fdata;
      ...
      if (rank == 0){
	//sprintf(mapoutstr, "%s/RelCalib%d", Param->info_Ringnumb.groupName,rloop+21);
	FILE *fp;
	char filn2[100];
...
void write_tfAS(double *S, fftw_complex *fdatas, long *datatopix, long *indpix,
  ...
  for (idet1=idetmin;idet1<=idetmax;idet1++){
	...
	rfGal = relfGal[idet1];
      ...
      deproject(S,indpix,datatopix+iseg*ns+ns*nseg*idet1,psip+iseg*ns+ns*nseg*idet1,ns,npix,Ps,polar,polard,polara,GalBP,rfGal);
...
void compute_ftrProcesdata(double *datas, fftw_complex *fdatas, long ns, long ndet, long iseg, long nseg, long idetr){
...
void do_PtNd(double *PNd, double *sumCross, fftw_complex *fdatas, long *datatopix, double *psip,
	     double *SpN_all, long ns, long ndet, long *indpix, long ipix_min, long npix,
	     bool CORRon, long iseg, long iseg_min, long nseg, unsigned char *segon, long lnr,
	     double *relCalib, double *Mp, long *hits, bool polar, double *polardet,
	     double *polarang, bool GalBP, double *relfGal, int idetr){
	...
	if (CORRon || idet2 == idet1){
	  ...
	  if (segon[idet2*lnr + iseg+iseg_min]){
		...
		Nk[ii] = SpN_all[ii+idet1*ns+idet2*ndet*ns];
	      ...
	      for (ii=0;ii<ns/2+1;ii++)
		Nk[ii] = SpN_all[ii+idet1*ns+idet2*ndet*ns]*relCalib[idet1]*relCalib[idet2];
	    ...
	    if ((Mp != NULL) && (idet2 == idet1)){
	      ...
	      //DO SOMETHING BETTER ABOUT POLAR AND LEAKAGE HERE
	      compute_diagPtNPCorr(Nk,datatopix+ns*iseg+ns*nseg*idet1,ns,indpix,ipix_min,npix,Mp);
	      if (polar){
		    for (ii=0;ii<npix;ii++){
	    ...
	    if (mpos >= 0){
	        ...
	        if (!polar) {
                ...
                if (sumCross != NULL){
                    ...
                    sumCross[npix+mpos] += sin(2.0*(psip[ppos] + polarang[idet1]));
                }
	            PNd[mpos] += Nd[ii];
	            PNd[npix + mpos] += polardet[idet1]*cos(2.0*(psip[ppos] + polarang[idet1]))*Nd[ii];
	            PNd[2*npix + mpos] += polardet[idet1]*sin(2.0*(psip[ppos] + polarang[idet1]))*Nd[ii];
	            if (GalBP)
      ...
      for (ii=0;ii<ns;ii++){
	    ...
	    if (mpos >= 0){
	      ...
	      if (!polar) {
	        ...
	        PNd[mpos] += 1;
	        PNd[npix + mpos] += polardet[idet1]*cos(2.0*(psip[ppos] + polarang[idet1]));
	        PNd[2*npix + mpos] += polardet[idet1]*sin(2.0*(psip[ppos] + polarang[idet1]));
            PNd[3*npix + mpos] += polardet[idet1]*cos(4.0*(psip[ppos] + polarang[idet1]));
            PNd[4*npix + mpos] += polardet[idet1]*sin(4.0*(psip[ppos] + polarang[idet1]));
	      }
	    }
      }
    }
...
void ang2pix_ring( const long nside, double theta, double phi, long *ipix) {
  ...
  */
  ...
  if( za <= z0 ) {
    ...
    kshift = 0;
    if ((ir & 1) == 0) kshift = 1;// ! kshift=1 if ir even, 0 otherwise
...
Compute the TOI from a map for ONE RING (ns samples) and ONE det using pointing info.
...
/*  compute_Nm1d
    ...
    double *SpN_all : the inverse noise spectrum (for the det and ring considered)
...
*/
void
compute_Nm1d (double *d, long ns, double *SpN_all, double relcal, double *Nm1d)
{
  long ii;
...
void fMPI_merge(double *lmap, double *d, long nsmpix, long npix, long npixtot,
		int rank, int newrank, int size, int nsubm,
		bool polar, bool GalBP, MPI_Status status, bool allprc, MPI_Comm newcomm){
  
  /// Each master node from subgroups send his submap to the master node. The master node contains all parts.
  // d is the input
  // lmap is the output  
  ...
  if ((newrank == 0) && (rank != 0))
    MPI_Send(d, nsmpix, MPI_DOUBLE, 0, rank, MPI_COMM_WORLD);///send submap to the master node
  ...
  if (rank == 0){
    ...
    for (idet=1;idet<nsubm;idet++){
      ...
      nns = ipixmax-ipixmin;
      MPI_Recv(lmap+ipixmin,nns, MPI_DOUBLE, idet*size/nsubm, idet*size/nsubm, MPI_COMM_WORLD,&status);
  ...
  if (polar){
	...
	ipixmax = (idet+1)*npix/nsubm;
	nns = ipixmax-ipixmin;
	MPI_Recv(lmap+npix+ipixmin,nns, MPI_DOUBLE, idet*size/nsubm, idet*size/nsubm, MPI_COMM_WORLD, &status);
      }
    }
    if ((newrank == 0) && (rank != 0)){
	...
	ipixmax = (idet+1)*npix/nsubm;
	nns = ipixmax-ipixmin;
	MPI_Recv(lmap+2*npix+ipixmin, nns, MPI_DOUBLE, idet*size/nsubm, idet*size/nsubm, MPI_COMM_WORLD, &status);
      }
    if (GalBP){
      if ((newrank == 0) && (rank != 0))
	...
	for (idet=1;idet<nsubm;idet++){
	  ...
	  ipixmax = (idet+1)*npix/nsubm;
	  nns = ipixmax-ipixmin;
	  MPI_Recv(lmap+3*npix+ipixmin, nns, MPI_DOUBLE, idet*size/nsubm, idet*size/nsubm, MPI_COMM_WORLD, &status);
	}
      }
    }
  } else 
...
void read_bolo_offsets(string bolo, string file_BoloOffsets, double *offsets){
  ...
  while (fscanf(fp, "%ld%ld%lf%lf%lf%s%s\n", &num,&pnum, &lel, &xel, &angl, tempchar, boloname) != EOF) {
    if (bolo == boloname) {
      nobolo = 0;
      offsets[0] = lel;
      offsets[1] = xel;
...
double MPI_compute_chi2(fftw_complex *fdatas0, fftw_complex *fodipole, fftw_complex *fdatas, fftw_complex *fdata, double *map, long nseg, long *datatopix, long *indpix, double *psip, double *relCalib, unsigned char *segon, double *SpN_all, long npix, long ns, long ndet, long iseg_min, long lnr, int nsubm, bool polar, double *polardet, double *polarang, bool GalBP, double *relfGal, long newrank){
  ...
  if (newrank != 0){
    for (iseg=0;iseg<nseg;iseg++){  
      ...
      write_tfAS(map,fdatas,datatopix,indpix,psip,npix,ns,ndet,iseg,iseg_min,nseg,
		 segon,lnr,polar,polardet,polarang,GalBP,relfGal,-1);
	...
	if (segon[idet*lnr + iseg+iseg_min]){
	  ...
	  for (ii=0;ii<ns/2+1;ii++){
	    ...
	    fdata[ii][0] = (fdatas0[posfdatas][0] - relCalib[posrelC]*(fdatas[ii+idet*ns][0] + fodipole[posfdatas][0]))*sqrt(SpN_all[ii+idet*ns+idet*ndet*ns]);
	    fdata[ii][1] = (fdatas0[posfdatas][1] - relCalib[posrelC]*(fdatas[ii+idet*ns][1] + fodipole[posfdatas][1]))*sqrt(SpN_all[ii+idet*ns+idet*ndet*ns]);
	  }
...
void forceLeakzero(double *PtNPmatStot, long nside, long *indpix, long ipixmin, long nsmpix, bool polar){
  ...
  for (ii=0;ii<12*nside*nside;ii++)
    if (((ii < 12*nside*nside/8) || (ii > 12*nside*nside*7/8)) && (indpix[ii] >= 0)){
      if (polar)
...
void pointingshift(double x, double y, long nn, double *thetap, double *phip, double *psip, double *theta, double *phi, double *psi){
  ...
  phi_   = atan2(y,x);
  for(ii=0;ii<nn;ii++){
    theta[ii]  = acos(sin(thetap[ii])*sin(theta_)*cos(phi_+psip[ii]) + cos(thetap[ii])*cos(theta_));
    phi[ii]    = atan2(-sin(theta_)*sin(phi_+psip[ii]) , (-cos(thetap[ii])*sin(theta_)*cos(phi_+psip[ii]) + sin(thetap[ii])*cos(theta_))) + phip[ii];
  }
